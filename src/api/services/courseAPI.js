  ...
  getProfessorCourses: async (professorId, year, semester) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/course-openings/professor/${professorId}?year=${year}&semester=${semester}`
      );
      if (!response.ok) {
        throw new Error('Failed to fetch professor courses');
      }
      const data = await response.json();
      console.log('Server response:', data); // 디버깅용 로그 추가
      
      return data.map(course => ({
        id: course.openingId,
        name: course.course.courseName,
        code: course.course.courseName,
